...
module.exports = class PluginInfo {
    ...
    isCompatible(version) {
        ...
        const minVersion = compatibility.minimumVersion || '0.0.0';
        const maxVersion = compatibility.maximumVersion || '99.99.99';
        const blacklist = compatibility.blacklist || [];
    ...
    getChangelog(language, version) {
        ...
        const useVersion = version || this.getCurrentVersion();
    ...
    addChangelog(changelog) {
        this.info.changelogs = this.info.changelogs || {};
        Object.keys(changelog).forEach((version) => {
            Object.keys(changelog[version]).forEach((locale) => {
                this.info.changelogs[locale] = this.info.changelogs[locale] || {};
                this.info.changelogs[locale][version] = changelog[version][locale];
            });
        });
    }
};
