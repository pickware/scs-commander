...
var ShopwareStoreClient = require('./shopware_store_client');
...
module.exports = class ShopwareStoreCommander {
    ...
    constructor(username) {
        this.client = new ShopwareStoreClient(username);
        this.accountData = {};
        this.statics = {};
    }
    ...
    findPlugin(pluginName, extraFields) {
        var initialPromise = (!this.accountData.plugins) ? this.loadAccountData() : Promise.resolve(this.accountData);
        return initialPromise.then(data => {
            ...
            if (extraFields && extraFields.length > 0) {
                return this._loadExtraPluginFields(data.plugins[pluginName], extraFields);
            }
    ...
    uploadPluginBinary(plugin, filePath) {
        var binaryName = filePath.split(/(\\|\/)/g).pop();
    ...
    requestBinaryReview(plugin) {
        ...
        var pollReviewStatus = (review) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                return this.client.GET(`plugins/${plugin.id}/reviews`).then(res => {
                    ...
                    var updatedReview = res.body.find(rev => {
                    ...
                    if (review.status.id !== 1) {
                        ...
                        // Max polls reached
                        throw new Error('Reviews is taking longer than expected. Please check the review status online at https:://account.shopware.de/');
                    }
        ...
        return this.client.POST(`plugins/${plugin.id}/reviews`, {}).then(res => {
            ...
            return pollReviewStatus(review).then(updatedReview => {
                return plugin;
            });
        });
    }
     ...
     * @return {Promise}
     */
    _loadExtraPluginFields(plugin, fields) {
        // Load all extra fields
        var extraFieldPromises = fields.map(field => {
            return this.client.GET(plugin[field]).then(res => {
                plugin[field] = res.body;
            });
        });

        return Promise.all(extraFieldPromises).then(results => {
            return plugin;
        });
    }
