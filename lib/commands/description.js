...
if (typeof Program.opts().locale === 'undefined') {
...
async function main() {
    try {
        ...
        let plugin = await commander.findPlugin(Program.opts().plugin);
        if (!plugin) {
            process.exit(1);
        }

        // Try to find info for locale
        const info = plugin.infos.find(data => data.locale.name === Program.opts().locale);
        if (!info) {
            console.error(Chalk.white.bgRed.bold(`Locale '${Program.opts().locale}' is not available!`));
        ...
        plugin = await commander.savePlugin(plugin, {
            retryCount: Program.opts().maxUpdateRetries,
        });

        const successMessage = `Description of plugin ${plugin.name} for locale '${Program.opts().locale}' successfully updated! \u{1F389}`;
