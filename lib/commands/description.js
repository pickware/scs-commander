...
Program
    ...
    .option('-i, --interactive', 'Show diff and interactively ask before updating')
    .option('--patch', 'Alias for --interactive')
    .parse(process.argv);
...
async function main() {
    ...
    try {
        ...
        if (Program.patch || Program.interactive) {
            if (info.description === description) {
                // Equal contents.
                console.log('No changes');
            } else {
                const oldName = backupFilePath;
