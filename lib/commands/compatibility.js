...
async function main() {
    try {
        ...
        // Update the minimum Shopware version compatiblity of all plugin binaries
        ...
        await util.sequentiallyAwaitEach(plugin.binaries, async (binary) => {
            ...
            if (semver.lt(Program.minVersion, minCompatibleVersion)) {
                // Add new version compatiblities to lower the minimum compatibility
                console.log(
                    `Lowering minimum compatible Shopware version of binary ${binary.version} to ` +
                    `${Program.minVersion}...`
                );
                ...
                console.log(
                    `Raising minimum compatible Shopware version of binary ${binary.version} to ` +
                    `${Program.minVersion}...`
                );
                ...
                console.log(
                    `Minimum compatible Shopware version of binary ${binary.version} already matches ` +
                    `${Program.minVersion}`
                );
            ...
            if (binary.compatibleSoftwareVersions.length === 0) {
                ...
                binary.compatibleSoftwareVersions = [
                    shopwareVersions.find(
                        version => version.selectable && semver.eq(version.name, Program.minVersion)
                    ),
                ];
            }
        ...
        const successMessage = `Mininum Shopware version compatibility of plugin ${plugin.name} changed to ` +
                               `${Program.minVersion}! \u{1F389}`;
