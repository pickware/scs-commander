...
if (typeof Program.opts().minVersion === 'undefined') {
...
async function main() {
    try {
        ...
        if (plugin.latestBinary.compatibleSoftwareVersions[0].major === 'Shopware 6') {
            console.log(
                `The plugin ${plugin.name} is only compatible with Shopware 6, which requires Shopware version `
                + 'constraints to be defined in its composer.json file. This is not supported yet.',
            );
            process.exit(1);
        }
        ...
        const successMessage = `Minimum Shopware version compatibility of plugin ${plugin.name} changed to ${Program.opts().minVersion}! \u{1F389}`;
