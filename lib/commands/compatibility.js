...
async function main() {
    try {
        ...
        let plugin = await commander.findPlugin(Program.plugin, ['binaries']);
        if (!plugin) {
            process.exit(1);
        }
        ...
        for (const binary of plugin.binaries) {
            ...
            if (semver.lt(Program.minVersion, minCompatibleVersion)) {
                ...
                binary.compatibleSoftwareVersions = binary.compatibleSoftwareVersions.concat(shopwareVersions.filter(
                    version => version.selectable && semver.gte(version.name, Program.minVersion) && semver.lt(version.name, minCompatibleVersion)
                ));
