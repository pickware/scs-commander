...
async function main() {
    try {
        ...
        if (plugin.latestBinary.compatibleSoftwareVersions[0].major === 'Shopware 6') {
            console.log(`The plugin ${plugin.name} is compatible with Shopware 6 and there minimum and maximum `
            + 'compatibilities are set via composer.json. This is not implemented yet.');
            process.exit(1);
        }
