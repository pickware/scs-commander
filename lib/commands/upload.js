...
fs.exists(filePath)
  ...
  .then(pluginInfo =>
    ...
    commander.findPlugin(pluginInfo.getName(), ['binaries', 'reviews'])
      .then(plugin => [plugin, pluginInfo]))
  .then((args) => {
    const plugin = args[0];
    const pluginInfo = args[1];
    // Make sure that the version of the passed binary does not exist yet
    const conflictingBinary = plugin.binaries.find(binary => semver.eq(binary.version, pluginInfo.getCurrentVersion()));
