...
// Define CLI
function optOnOffAuto(suppliedValue) {
...
Program
    ...
    .option(
        ...
        'whether the Store should automatically ionCube-encode the released binary (auto retains previous release\'s setting, or sets to on)',
        optOnOffAuto,
        'auto'
    )
    .option(
        '--license-check-required <on|off|auto>',
        'sets the "License check required" flag in the Store (auto retains previous release\'s setting, or sets to on)',
        optOnOffAuto,
        'auto'
    )
...
if (Program.storeIoncubeEncode !== true && Program.storeIoncubeEncode !== false && Program.storeIoncubeEncode !== 'auto') {
    console.error(Chalk.white.bgRed.bold(`Invalid setting ${Program.storeIoncubeEncode} for --store-ioncube-encode.`));
    process.exit(1);
}
if (Program.licenseCheckRequired !== true && Program.licenseCheckRequired !== false && Program.licenseCheckRequired !== 'auto') {
    console.error(Chalk.white.bgRed.bold(`Invalid setting ${Program.storeIoncubeEncode} for --license-check-required.`));
    process.exit(1);
}
...
async function main() {
    ...
    try {
        ...
        if (compatibleVersionStrings.length > 0) {
            console.error(`Set shopware version compatiblity: ${compatibleVersionStrings.join(', ')}`);
        } else {
            console.error(
        ...
        if (Program.storeIoncubeEncode === true || Program.storeIoncubeEncode === false) {
            plugin.latestBinary.ionCubeEncrypted = Program.storeIoncubeEncode;
        } else if (Program.storeIoncubeEncode === 'auto') {
            // If the ionCubeEncrypted option was not set on the latest binary, set it to true.
            // Otherwise, retain the current setting.
            if (!('ionCubeEncrypted' in plugin.latestBinary)) {
                plugin.latestBinary.ionCubeEncrypted = true;
            }
        }
        if (Program.licenseCheckRequired === true || Program.licenseCheckRequired === false) {
            plugin.latestBinary.licenseCheckRequired = Program.licenseCheckRequired;
        } else if (Program.licenseCheckRequired === 'auto') {
            // If the licenseCheckRequired option was not set on the latest binary, set it to true.
            // Otherwise, retain the current setting.
            if (!('licenseCheckRequired' in plugin.latestBinary)) {
                plugin.latestBinary.licenseCheckRequired = true;
            }
        }
