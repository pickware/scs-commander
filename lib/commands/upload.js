...
fs.exists(filePath)
  ...
  .then((pluginInfo) => {
    ...
    const parser = new PluginChangelogParser();
  ...
  .then(pluginInfo =>
    ...
    commander
      .findPlugin(pluginInfo.getName(), ['binaries', 'reviews'])
      .then(plugin => [plugin, pluginInfo]))
  .then((args) => {
    const plugin = args[0];
    const pluginInfo = args[1];
    ...
    if (conflictingBinary) {
      if (Program.force) {
        return commander.updatePluginBinary(plugin, conflictingBinary, filePath)
          .then(updatedPlugin => [updatedPlugin, pluginInfo]);
      }

      throw new Error(`The binary version ${conflictingBinary.version} you're trying to upload already exists for ` +
          `plugin ${plugin.name}`);
    }

    // Upload the binary
    return commander.uploadPluginBinary(plugin, filePath).then(updatedPlugin => ({ updatedPlugin, pluginInfo }));
  })
  .then(({ updatedPlugin: plugin, pluginInfo }) => {
    ...
    if (compatibleVersionStrings.length > 0) {
      ...
      console.log(
        Chalk.yellow.bold(
          '\u{26A0} Warning: The plugin\'s compatibility constraints don\'t match any available shopware versions!'
        )
      );
    }
  ...
  .then(({ updatedPlugin: plugin, pluginInfo }) => {
    ...
    return commander.requestBinaryReview(plugin).then(updatedPlugin => ({ updatedPlugin, pluginInfo }));
  })
  .then(({ updatedPlugin: plugin, pluginInfo }) => {
    // Check review status
    ...
    if (review.status.name !== 'approved') {
      throw new Error(
        `The review of ${plugin.name} v${plugin.latestBinary.version} finished with status '${review.status.name}':` +
        `\n\n${review.comment}`
      );
    }

    const successMessage =
      `Review succeeded! Version ${plugin.latestBinary.version} of plugin ${plugin.name} is now available in the ` +
      'store. \u{1F389}';
