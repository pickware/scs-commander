...
Program
    ...
    .option('-p, --plugin <plugin>', 'The name of the plugin, for which the binary shall be uploaded.')
...
fs.exists(filePath).then(exists => {
    ...
    if (!pluginInfo) {
        throw new Error(`Cannot upload binary for plugin ${plugin.name}, because it is missing a plugin.json file.`);
    }

	// Try to find the plugin
    return commander.findPlugin(Program.plugin, ['binaries', 'reviews']).then(plugin => {
