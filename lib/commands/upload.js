...
fs.exists(filePath).then(exists => {
    ...
    var uploadSuccessMessage = `New version ${plugin.latestBinary.version} of plugin ${plugin.name} uploaded!`;
    console.log(Chalk.green.bold(uploadSuccessMessage));
    if (!Program.release) {
        util.showGrowlIfEnabled(uploadSuccessMessage);
        console.log(Chalk.yellow.bold(`Don't forget to manually release the version by requesting a review (try passing '--release' next time)`));
        process.exit(0);
    }
    ...
    var successMessage = `Review succeeded! Version ${plugin.latestBinary.version} of plugin ${plugin.name} is now available in the store.`;
