...
.then(({ commander, exists }) => {
    ...
    return reader.readZip(filePath).then(pluginInfo => ({ commander, pluginInfo }));
})
.then(({ commander, pluginInfo }) => {
    if (!pluginInfo) {
    ...
    return parser.readZip(filePath)
        .then(pluginChangeLog => ({ commander, pluginInfo, pluginChangeLog }));
})
.then(({ commander, pluginInfo, pluginChangeLog }) => {
    if (pluginChangeLog) {
    ...
    return commander.findPlugin(pluginInfo.getName(), ['binaries', 'reviews'])
        .then(plugin => ({ commander, plugin, pluginInfo }));
})
.then(({ commander, plugin, pluginInfo }) =>
    // Enable partial ionCube encryption (only applies, if the plugin will be encrypted)
     commander.enablePartialIonCubeEncryption(plugin)
        .then(updatedPlugin => ({ plugin: commander, updatedPlugin, pluginInfo })))
.then(({ commander, plugin, pluginInfo }) => {
    ...
    return commander.uploadPluginBinary(plugin, filePath)
        .then(updatedPlugin => ({ commander, plugin: updatedPlugin, pluginInfo }));
})
.then(({ commander, plugin, pluginInfo }) => {
