...
const getShopwareCompatibility = (plugin) => {
    ...
    return plugin.latestBinary.compatibleSoftwareVersions.map(shopwareVersion => shopwareVersion.name).sort(semver.compare).shift();
};
...
commander.loadAccountData()
    .then((data) => {
        var plugins = Object.keys(data.plugins).map(pluginName => data.plugins[pluginName]);
        if (!Program.showAll) {
