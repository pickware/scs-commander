...
module.exports = class ShopwareStoreCommander {
    ...
    async uploadPluginBinary(plugin, filePath) {
        ...
        const res = await this.client.post(`plugins/${plugin.id}/binaries`, filePath, {
            headers: {
            ...
            transformRequest: [
                (filePath2) => {
                    ...
                    data.append('file', fs.createReadStream(filePath2), { filename: path.basename(filePath2) });
    ...
    async updatePluginBinary(plugin, binary, filePath) {
        ...
        const res = await this.client.post(`plugins/${plugin.id}/binaries/${binary.id}/file`, filePath, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
            transformRequest: [
