...
module.exports = class ShopwareStoreCommander {
    ...
    async uploadPluginBinary(plugin, filePath) {
        ...
        const { data, headers } = await getPostData(filePath);
    ...
    async getSalesForPlugin(plugin, lastSale) {
        ...
        do {
            ...
            for (let i = 0; i < tmpArr.length; i += 1) {
                const sale = tmpArr[i];
                if (lastSale &&
                    sale.creationDate === lastSale.bookingDate &&
                    (sale.priceModel.price ? sale.priceModel.price : 0) === lastSale.amount &&
                    sale.licenseShop.domain === lastSale.domain) {
                    exit = true;
                    break;
                } else {
                    sales.push(sale);
    ...
    /**
     * @param {Object} plugin
     * @param {Object} lastBrokerage to compare against for db sync - optional
     * @return {Promise}
     */
    async getBrokeragesForPlugin(plugin, lastBrokerage) {
        const limit = 2000;
        const brokerages = [];
        let currOffset = 0;
        let tmpArr = [];
        ...
        do {
            ...
            for (let i = 0; i < tmpArr.length; i += 1) {
                const brokerage = tmpArr[i];
                if (lastBrokerage &&
                    brokerage.bookingDate === lastBrokerage.bookingDate &&
                    brokerage.amount === lastBrokerage.amount &&
                    brokerage.orderNumber === lastBrokerage.orderNumber &&
                    brokerage.text === lastBrokerage.text &&
                    brokerage.status.name === lastBrokerage.status &&
                    brokerage.type.name === lastBrokerage.type &&
                    brokerage.domain === lastBrokerage.domain) {
                    exit = true;
                    break;
                } else {
